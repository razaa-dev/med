      ...
      <div className={`flex-1 transition-all duration-300 ${isSidebarOpen ? 'ml-64' : 'ml-0 sm:ml-20'}`}>
        ...
        <main className="p-6 bg-gray-100 flex-grow">
          <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Main Account Card */}
            <div className="bg-white p-6 rounded-lg shadow-lg col-span-1 md:col-span-2 lg:col-span-2">
              <div className="flex justify-between items-center">
                <div>
                  <h2 className="text-gray-500 font-bold">Main account</h2>
                ...
                <div className="text-right">
                  ...
                  <h1 className="text-4xl font-bold">{NAMES.NairaSymbol}{NAMES.WALLETBALANCE.toLocaleString()}</h1>
                </div>
              </div>
              <div className="mt-4 flex space-x-4">
                <button className="bg-secondary text-white px-4 py-2 rounded-lg"><a href="/institution/pharmacy/wallet">Withdraw money</a></button>
                <button className="bg-primary text-white px-4 py-2 rounded-lg"><a href="/institution/pharmacy/wallet">Link accounts</a></button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
            ...
            <PharmacyCard pharmacy="Total Customers" balance={userData?.data?.no_of_consultations.toLocaleString()} icon={<FaUserCheck size={40} className='text-primary'/>} />
          </div>
...
const TransactionList = ({ transactions }) => ( // Ensure prop name matches
  <div className="space-y-4 hidden md:block">
    <div className="">
      <table className="min-w-full table-auto text-left">
        ...
        <tbody>
          {transactions.map((order, index) => ( // Ensure you're mapping over the correct prop
